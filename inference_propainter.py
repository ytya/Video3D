...
def read_mask(mpath, length, size, flow_mask_dilates=8, mask_dilates=5):
    ...
    return flow_masks, masks_dilated, masks_img
...
if __name__ == "__main__":
    ...
    src_frames = [np.array(f) for f in frames]

    frames, size, out_size = resize_frames(frames, size)
    out_size = src_frames[0].shape[:2][::-1]
    ...
    if args.mode == "video_inpainting":
        ...
        flow_masks, masks_dilated, masks_img = read_mask(
            args.mask, frames_len, size, flow_mask_dilates=args.mask_dilation, mask_dilates=args.mask_dilation
        )
        w, h = size
    ...
    for f, c, m in zip(src_frames, comp_frames, masks_img):
        m = np.asarray(m)
        f[m == 255] = c[m == 255]
    imageio.mimwrite(os.path.join(save_root, "masked_in.mp4"), masked_frame_for_save, fps=fps, quality=7)
    imageio.mimwrite(os.path.join(save_root, "inpaint_out.mp4"), src_frames, fps=fps, quality=7)
    imageio.mimwrite(os.path.join(save_root, "inpaint_comp.mp4"), comp_frames, fps=fps, quality=7)
