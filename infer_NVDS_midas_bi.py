...
from nvds.backbone import *
from nvds.full_model import *
from nvds.gmflow.geometry import *
from nvds.gmflow.gmflow import GMFlow

# from NVDS.dpt.models import DPTDepthModel
# from NVDS.dpt.midas_net import MidasNet_large
from nvds.networks import *
from nvds.smooth_loss import *
...
def get_args_parser():
    ...
    parser.add_argument("--all_seq_len", type=int, default=4, help="all sequence length for training")
    parser.add_argument("--max_epoch", default=500, help="max epochs for training")
    parser.add_argument("--vnum", default="000423", type=str)
    parser.add_argument("--timesall", default=2, type=int)
    parser.add_argument("--base_dir", default="/xxx/xxx/", type=str)
    parser.add_argument("--infer_w", default="896", type=int)
    parser.add_argument("--infer_h", default="384", type=int)
...
if __name__ == "__main__":
    ...
    args.resume = "./NVDS/gmflow/checkpoints/gmflow_sintel-0c07dcb3.pth"
    ...
    # dpt = MidasNet_large('./NVDS/dpt/checkpoints/midas_v21_384.pt', non_negative=True).to(device_flow)
    dpt = torch.hub.load("intel-isl/MiDaS", "DPT_BEiT_L_512", pretrained=True, non_negative=True).to(device)
    dpt.eval()

    seq_len = 4
    clip_step = 1
    video_dir = "./NVDS/demo_videos/" + args.vnum + "/"
    ...
    for i in range(len(frames)):
        ...
        with torch.no_grad():
            ...
            cv2.imwrite(
                save_disp_dir + "/frame_%06d" % (i) + ".png",
        ...
        for i in range(len(frames)):
            ...
            with torch.no_grad():
                ...
                if times % 2 == 0:
                    ...
                    plt.imsave(
                        save_ksh_dir + str(len(frames) - i - 1) + ".png",
                        outputs.cpu().numpy().squeeze(),
                        cmap="inferno",
                        vmin=np.min(outputs.cpu().numpy().squeeze()),
                        vmax=np.max(outputs.cpu().numpy().squeeze()),
                ...
                if times % 2 == 0:
                    ...
                    cv2.imwrite(
                        save_disp_dir + "/frame_%06d" % (len(frames) - i - 1) + ".png",
                        outputs.astype("uint16"),
                        [cv2.IMWRITE_PNG_COMPRESSION, 0],
                    )
    ...
    for i in range(len(frames)):
        ...
        outputs = 65535.0 * (outputs - depth_min) / (depth_max - depth_min)
        cv2.imwrite(
            save_disp_dir + "/frame_%06d" % (i) + ".png", outputs.astype("uint16"), [cv2.IMWRITE_PNG_COMPRESSION, 0]
        )
